...
export default function LiveVoiceRoom() {
  ...
  const {
    ...
    voice: "Zephyr",
    systemInstruction:
      "You are an intelligent, friendly conversational partner in a live video call. Answer questions directly, keep responses concise, and converse naturally in real-time.",
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[600px] bg-slate-950 text-slate-100 p-6 rounded-2xl border border-slate-800">
      <div className="w-full max-w-2xl bg-slate-900 border border-slate-800 rounded-3xl p-8 flex flex-col items-center shadow-2xl relative overflow-hidden">
        ...
        <div className="relative w-40 h-40 flex items-center justify-center my-6">
          <div
            ...
            style={{
              transform: `scale(${
                1 + (isAiSpeaking ? aiVolume * 2 : userVolume * 1.2)
              })`,
          ...
          <div className="relative z-10 w-28 h-28 rounded-full bg-slate-950 border border-cyan-500/30 flex items-center justify-center shadow-inner">
            <Sparkles
              className={`w-10 h-10 ${
                isAiSpeaking ? "text-cyan-400 animate-spin" : "text-slate-400"
              }`}
            />
          </div>
        </div>
        ...
        <div className="w-full min-h-[60px] bg-slate-950/60 rounded-xl border border-slate-800/80 p-4 text-center text-sm text-slate-300 mb-6 flex items-center justify-center">
          {currentCaption ? (
            ...
            <span className="text-slate-600">
              Live subtitles and transcript will appear here...
            </span>
          )}
        </div>
        ...
        <video
          ...
          playsInline
          muted
          className={`w-48 h-32 rounded-xl object-cover mb-6 border border-slate-800 ${
