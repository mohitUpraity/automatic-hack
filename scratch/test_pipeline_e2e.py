...
def test_pipeline():
    print(f"--- 1. Testing Document Upload for {USER_ID} ---")
    files = {"file": ("Mohit_Resume.md", sample_resume, "text/markdown")}
    data = {"user_id": USER_ID, "doc_type": "resume"}
    res = requests.post(f"{API_BASE}/api/documents/upload", files=files, data=data)
    print("Upload status:", res.status_code)
    print("Upload response:", res.json())
    assert res.status_code == 200

    print(f"\n--- 2. Testing Opportunities Feed for {USER_ID} ---")
    opp_res = requests.get(f"{API_BASE}/api/opportunities?candidate_id={USER_ID}")
    print("Opportunities status:", opp_res.status_code)
    opps = opp_res.json().get("opportunities", [])
    print(f"Total Opportunities Found: {len(opps)}")
    if opps:
        print("Top 3 Matched Opportunities:")
        for o in opps[:3]:
            print(f"  - [{o.get('relevance_score')}% fit] {o.get('title')} @ {o.get('company')}")
    assert len(opps) > 0

    print(f"\n--- 3. Testing Knowledge Graph for {USER_ID} ---")
    kg_res = requests.get(f"{API_BASE}/api/knowledge-graph/{USER_ID}")
    ...
    print("Node Breakdown by Category:", node_types)
    assert len(nodes) > 0
    assert len(edges) > 0

    print(f"\n--- 4. Testing Candidate Details for {USER_ID} ---")
    cand_res = requests.get(f"{API_BASE}/api/candidates/{USER_ID}")
    print("Candidate details status:", cand_res.status_code)
    cand = cand_res.json().get("candidate", {})
    print(f"Candidate Name: {cand.get('name')}")
    print(f"Candidate Skills ({len(cand.get('skills', []))}): {cand.get('skills', [])[:5]}")
    print(f"Candidate Projects ({len(cand.get('projects', []))}): {cand.get('projects', [])}")

    print("\n✅ PIPELINE FULLY VERIFIED AND WORKING 100%!")
